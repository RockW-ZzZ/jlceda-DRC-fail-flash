...
- **定时保存**：可设置每 1 / 5 / 10 / 30 / 60 分钟自动保存当前激活文档（原理图 / PCB / 面板）。
- **定时DRC检查**：可设置每 1 / 5 / 10 / 30 / 60 分钟自动执行 DRC 检查并语音播报结果；可单独开关是否呼出 EDA 自带 DRC 窗口。
- 语音播报：
...
4. 按需设置：
   ...
   - **定时保存**：选择周期后，插件会在 EDA 中按周期自动保存当前激活文档。
   - **定时DRC检查**：选择周期后，插件会按周期自动执行 DRC 检查并播报结果；勾选「定时DRC呼出窗口」则同时显示 EDA 自带 DRC 窗口。
